...
auto DisplayAdapter::findCrtcForEncoder(const drmModeRes *resources, const drmModeEncoder *encoder) -> uint32_t {
	uint32_t idx = 0;
...
auto DisplayAdapter::findCrtcForConnector(const drmModeRes *resources, const drmModeConnector *connector) const -> uint32_t {
	int idx = 0;
